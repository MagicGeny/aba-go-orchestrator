...
func (r *PostgresRepository) getNextPendingTarget(ctx context.Context, tenantID uuid.UUID, warm bool) (*domain.PendingTargetForDosing, error) {
	// Warm = pending target that already has a chat_id mapping (repeat contact).
	// Cold = pending target with no mapping; doseTenant applies the 07:00–21:00
	// work window, daily cold limit, and inter-send interval before enqueue.
	mappingClause := `
		AND EXISTS (
